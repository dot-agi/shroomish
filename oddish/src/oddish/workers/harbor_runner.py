...
from oddish.config import BEDROCK_ENV_VARS, looks_like_bedrock_model_id
...
def _scoped_bedrock_env(model: str | None) -> Iterator[None]:
    ...
    so with the Bedrock env vars set globally every claude-code trial defaults
    to Bedrock.  When the trial's model id does not look Bedrock-native (ARNs,
    ``anthropic.*`` ids, or region-prefixed inference profiles), we temporarily
    unset the Bedrock signals for the process so Harbor falls back to
    ``ANTHROPIC_API_KEY``.
    ...
    previous: dict[str, str] = {
        name: os.environ.pop(name) for name in BEDROCK_ENV_VARS if name in os.environ
    }
    try:
        yield
    finally:
